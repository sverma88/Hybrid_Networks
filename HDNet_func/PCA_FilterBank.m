...
img = im2col_mean_removal(InImg{1},[PatchSize PatchSize]);

for i= 1:numel(RandIdx) %1:ImgZ
    im = im2col_mean_removal(InImg{RandIdx(i)},[PatchSize PatchSize]); % collect all the patches of the ith image in a matrix, and perform patch mean removal
    Rx = Rx + im*im'; % sum of all the input images' covariance matrix
end

Rx = Rx/(NumRSamples*size(img,2));
[E D] = eig(Rx);
