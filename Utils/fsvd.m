...
%   Copyright 2011 Ismail Ari, http://ismailari.com.
    ...
    % Form a real n¡Ñl matrix G whose entries are iid Gaussian r.v.s of zero
    ...
    if nargin >= 4 && usePowerMethod
        ...
        % Compute the m¡Ñl matrices H^{(0)}, ..., H^{(i)}
        ...
        % Form the m¡Ñ((i+1)l) matrix H
        H = cell2mat(H);
    end

    % Using the pivoted QR-decomposiion, form a real m¡Ñ((i+1)l) matrix Q
    % whose columns are orthonormal, s.t. there exists a real
    % ((i+1)l)¡Ñ((i+1)l) matrix R for which H = QR.  
    % XXX: Buradaki column pivoting ile yap?lmayan hali.
    [Q,~] = qr(H,0);

    % Compute the n¡Ñ((i+1)l) product matrix T = A^T Q
    T = A'*Q;

    % Form an SVD of T
    [Vt, St, W] = svd(T,'econ');

    % Compute the m¡Ñ((i+1)l) product matrix
    Ut = Q*W;

    % Retrieve the leftmost m¡Ñk block U of Ut, the leftmost n¡Ñk block V of
    % Vt, and the leftmost uppermost k¡Ñk block S of St. The product U S V^T
    % then approxiamtes A. 
